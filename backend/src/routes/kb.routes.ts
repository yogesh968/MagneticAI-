...
const uploader = multer({ dest: "uploads/", limits: { fileSize: 10 * 1024 * 1024 }, fileFilter: (_req, file, cb) => cb(null, allowed.has(file.mimetype) || /\.(txt|md)$/i.test(file.originalname)) });
